...
class OffboardControl(Node):
    ...
    def __init__(self):
        ...
        self.r = self.config['helix']['radius']
        self.ns = 5*self.config['helix']['num_loops']
        self.hh = self.config['helix']['hheight']
        height = self.config['helix']['height']/self.ns
        self.hspeed = self.config['helix']['hspeed']
        self.vspeed = self.config['helix']['vspeed']
        self.xc = sp[0]
        self.yc = sp[1]
        self.setpoint_targets = [sp]
        self.setpoint_targets.append((sp[0], sp[1], sp[2]+self.hh, sp[3]+0.00))
        ...
        for t in range(self.ns):
            xt = (self.r)*cos(2*pi*(((t+1))/5))
            yt = (self.r)*sin(2*pi*(((t+1)/5)))
            zt = self.hh-height*self.ns+height*t
            self.setpoint_targets.append((sp[0]+xt, sp[1]+yt, sp[2]+zt, sp[3]+0.00))
        self.setpoint_targets.append((sp[0], sp[1], sp[2]-self.hh, sp[3]+0.00))
    ...
    def tpol(self,x1,y1,x2,y2,x3,y3,d):
        ...
        elif self.offboard_setpoint_counter_ == 2*self.ns+1:
            return [x3+d*self.gd(x1,x3),y1]
        elif self.offboard_setpoint_counter_> 2*self.ns+1:
            return [x1,y1]
        else:
            return self.tpoc(self.xc,self.yc,self.r,x1,y1,x3,y3,d)
    def tpol2(self,z1,z2,z3,d):
        distance = abs(z2-z1)
        if distance<=2*d:
            return z1
        elif self.offboard_setpoint_counter_== 0:
            ...
            if abs(z2)>abs(z1):
                    return z1-0.005*self.gd(z1,z3)
            elif abs(z2)<abs(z3):
                    return z3+0.005*self.gd(z1,z3)
            else:
                return z2+d*self.gd(z1,z3)
        elif abs(z3)>abs(z1):
            if abs(z2)<abs(z1):
                    return z1-0.005*self.gd(z1,z3)
            elif abs(z2)>abs(z3):
    ...
    def publish_trajectory_setpoint(self, pose1:tuple, pose2:tuple):
        msg = TrajectorySetpoint()
        posef = (self.tpol(pose2[0], pose2[1], pose1[0], pose1[1], self.pose.pose.position.x, self.pose.pose.position.y,self.hspeed))
        posef2 = (self.tpol2(pose2[2],self.pose.pose.position.z,pose1[2],self.vspeed))
        msg.x, msg.y, msg.z = posef[0], posef[1], posef2
        msg.yaw = pose2[3]
        msg.timestamp = self.time()
