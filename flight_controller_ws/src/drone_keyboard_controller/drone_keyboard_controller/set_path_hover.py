"""
ROS2 Node for sending trajectory setpoints for hover (simply run the node when placing drone at starting point, sp)
...
class OffboardControl(Node):
    ...
    def __init__(self):
        ...
        hx = self.config['hover']['hx']
        hy = self.config['hover']['hy']
        hz = self.config['hover']['hz']
        hyaw = self.config['hover']['hyaw']
        self.setpoint_targets = [sp,
                                 (sp[0]+hx, sp[1]+hy, sp[2]+hz, sp[3]+hyaw)]
        self.bind = 'z'   
