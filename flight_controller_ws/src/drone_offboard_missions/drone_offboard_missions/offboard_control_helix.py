...
class OffboardControl(Node):
    ...
    def __init__(self):
        ...
        self.setpoint_targets.append((sp[0], sp[1], sp[2]-0.20, sp[3]+0.00))
        for t in range(self.ns):
            xt = (self.r)*cos(2*pi*(2*t/self.ns))
            yt = (self.r)*sin(2*pi*(2*t/self.ns))
            zt = -0.20-(height)*t
            self.setpoint_targets.append((sp[0]+xt, sp[1]+yt, sp[2]+zt, sp[3]+0.00))
        for t in range(self.ns):
            xt = (self.r)*cos(2*pi*((2*(t+1))/self.ns))
            yt = (self.r)*sin(2*pi*((2*(t+1))/self.ns))
            zt = -0.20-height*self.ns+height*t
            self.setpoint_targets.append((sp[0]+xt, sp[1]+yt, sp[2]+zt, sp[3]+0.00))
        self.setpoint_targets.append((sp[0], sp[1], sp[2]-0.20, sp[3]+0.00))
    ...
    def gd2(self,xc,yc,x1,y1,x2,y2):
        ...
        self.get_logger().info(str(atp1)+" "+str(atp2))
    ...
    def point_along_circle(self, center_x, center_y, radius, point_x, point_y, target_x, target_y, distance):
        ...
        self.get_logger().info(str(angle_to_point))
        return new_x, new_y
